...
export function VendorStores() {
  ...
  useEffect(() => {
    ...
    const loadStores = async () => {
      // Don't set loading true if it's already loading
      if (!user?.id) return;
      ...
      try {
        ...
        const data = await response.json();
        if (active) setStores(data.stores || []);
      } catch (error: any) {
        if (!active || error.name === 'AbortError' || error === 'Duplicate request prevention') return;
        console.error('Fetch stores error:', error);
        showToast.error('Failed to load stores');
      } finally {
